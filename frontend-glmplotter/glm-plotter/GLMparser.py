    ...
    if verb:
        ...
        print(
            ...
            + str(
                [
                    str(a[0]) + ": " + str(a[1])
                    for a in zip(countClass.keys(), countClass.values())
                ]
            )
        )
...
def readClock(lines, currLine):
    ...
    while True:
        ...
        if lines[currLine].strip().startswith("timezone"):
            timezone = (
                lines[currLine].strip().split(";")[0].strip("timezone ").strip('"')
            )
...
def getAieul(objs, name):
    ...
    if len(parent) == 0:
        parent = [
            obj for obj in objs if "name_oldGLM" in obj and obj["name_oldGLM"] == name
        ]
    if len(parent) == 0:
...
def createD3JSON(objs, fileNm_out=""):
    ...
    links = list(
        zip(
            [getAieul(objs, link["from"])[0]["name"] for link in link_objs],
    ...
    children = dict(
        [(obj["parent"], obj["class"]) for obj in objs if obj["class"] in child_type]
    )
    # find unique nodes
    unique_nodes = list(
        set(
            [n1 for n1, n2, n3 in links]
            + [n2 for n1, n2, n3 in links]
    ...
    if len(unique_nodes) > len(node_objs):
        print(
            "I had to add "
            + str(len(unique_nodes) - len(node_objs))
    ...
    if len(unique_nodes) > 0:
        ...
        JSONstr += (
            '    {"name":"'
            + unique_nodes[len(unique_nodes) - 1]
            + '","classNm":"'
            + str(classNm[len(unique_nodes) - 1])
            + '","child":"'
            + str(child[len(unique_nodes) - 1])
            + '"}\n'
        )
    JSONstr += '  ],\n "links":[\n'
    if len(links) > 0:
